...
// node_modules/@actions/github/node_modules/node-fetch/lib/index.js
var require_lib3 = __commonJS({
  "node_modules/@actions/github/node_modules/node-fetch/lib/index.js"(exports, module2) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    ...
    Inline = function() {
      ...
      Inline2.parse = function(value, exceptionOnInvalidType, objectDecoder) {
        ...
        switch (value.charAt(0)) {
          case "[":
            result = this.parseSequence(value, context2);
            ++context2.i;
            break;
          case "{":
            result = this.parseMapping(value, context2);
            ++context2.i;
            break;
          default:
            result = this.parseScalar(value, null, ['"', "'"], context2);
        }
        if (this.PATTERN_TRAILING_COMMENTS.replace(value.slice(context2.i), "") !== "") {
          throw new ParseException('Unexpected characters near "' + value.slice(context2.i) + '".');
        }
        return result;
      };
      ...
      Inline2.parseScalar = function(scalar, delimiters, stringDelimiters, context2, evaluate) {
        ...
        if (ref = scalar.charAt(i), indexOf.call(stringDelimiters, ref) >= 0) {
          output = this.parseQuotedScalar(scalar, context2);
          i = context2.i;
          if (delimiters != null) {
          ...
          if (evaluate) {
            output = this.evaluateScalar(output, context2);
          }
        }
        context2.i = i;
        return output;
      };
      Inline2.parseQuotedScalar = function(scalar, context2) {
        var i, match, output;
        i = context2.i;
      ...
      Inline2.parseSequence = function(sequence, context2) {
        ...
        while (i < len) {
          ...
          switch (sequence.charAt(i)) {
            case "[":
              output.push(this.parseSequence(sequence, context2));
              i = context2.i;
              break;
            case "{":
              output.push(this.parseMapping(sequence, context2));
              i = context2.i;
              break;
            case "]":
              return output;
            ...
            default:
              ...
              value = this.parseScalar(sequence, [",", "]"], ['"', "'"], context2);
              i = context2.i;
      ...
      Inline2.parseMapping = function(mapping, context2) {
        ...
        while (i < len) {
          ...
          while (i < len) {
            ...
            switch (mapping.charAt(i)) {
              case "[":
                value = this.parseSequence(mapping, context2);
                i = context2.i;
                ...
                break;
              case "{":
                value = this.parseMapping(mapping, context2);
                i = context2.i;
                if (output[key] === void 0) {
                  output[key] = value;
                }
              ...
              case "\n":
                break;
              default:
                value = this.parseScalar(mapping, [",", "}"], ['"', "'"], context2);
                i = context2.i;
                if (output[key] === void 0) {
                  output[key] = value;
                }
      ...
      Inline2.evaluateScalar = function(scalar, context2) {
              ...
              case "!":
                ...
                switch (firstWord) {
                  ...
                  default:
                    ...
                    objectDecoder = context2.objectDecoder, exceptionOnInvalidType = context2.exceptionOnInvalidType;
                    if (objectDecoder) {
  ...
  "node_modules/yamljs/lib/Parser.js"(exports, module2) {
    ...
    Parser = function() {
      ...
      Parser2.prototype.parse = function(value, exceptionOnInvalidType, objectDecoder) {
        var alias, allowOverwrite, block, c, context2, data, e, first, i, indent, isRef, j, k, key, l, lastKey, len, len1, len2, len3, lineCount, m, matches, mergeNode, n, name, parsed, parsedItem, parser, ref, ref1, ref2, refName, refValue, val, values;
        if (exceptionOnInvalidType == null) {
          exceptionOnInvalidType = false;
        }
        if (objectDecoder == null) {
        ...
        while (this.moveToNextLine()) {
          ...
          if (values = this.PATTERN_SEQUENCE_ITEM.exec(this.currentLine)) {
            if (this.CONTEXT_MAPPING === context2) {
              throw new ParseException("You cannot define a sequence item when in a mapping");
            }
            context2 = this.CONTEXT_SEQUENCE;
            if (data == null) {
              data = [];
            }
            ...
            if (this.CONTEXT_SEQUENCE === context2) {
              throw new ParseException("You cannot define a mapping item when in a sequence");
            }
            context2 = this.CONTEXT_MAPPING;
            if (data == null) {
              data = {};
            }
...
var import_yamljs = __toESM(require_Yaml());
main().catch((err) => console.error(err));
function main() {
  return __async(this, null, function* () {
    var _a;
    const githubApiKey = (0, import_core.getInput)("github-token") || process.env.GITHUB_TOKEN;
    const repository = (0, import_core.getInput)("repository") || process.env.GITHUB_REPOSITORY;
    const service = (0, import_core.getInput)("service") || void 0;
    const path = (0, import_core.getInput)("path") || "service.datadog.yaml";
    const octokit = (0, import_github.getOctokit)(githubApiKey);
    console.log(`looking up datadog service definition "${path}" at repository "${repository}"`);
    const file = yield getFile(octokit, repository, path, import_github.context.ref || void 0);
    const datadog = {
      service: "",
      team: "",
      module: "",
    ...
    if (file) {
      ...
      if (json) {
        const tags = Object.fromEntries(Array.from((_a = json["tags"]) != null ? _a : []).map((entry) => typeof entry == "string" ? entry.split(":") : []));
        ...
        datadog.tags = Object.entries(tags).map(([key, value]) => `${key}:${value}`);
      }
    }
    (0, import_core.setOutput)("service", datadog.service);
    (0, import_core.setOutput)("team", datadog.team);
    (0, import_core.setOutput)("module", datadog.module);
    if (datadog.service) {
      (0, import_core.exportVariable)("DD_SERVICE", datadog.service);
      (0, import_core.exportVariable)("TFSO_DD_SERVICE", datadog.service);
    }
    if (datadog.tags.length > 0) {
      (0, import_core.exportVariable)("DD_TAGS", datadog.tags.join(","));
      (0, import_core.exportVariable)("TFSO_DD_TAGS", datadog.tags.join(","));
    }
  });
}
function getFile(octokit, repository, path, ref) {
  return __async(this, null, function* () {
...
function getJson(multipartYaml, service) {
  for (const part of multipartYaml.split(/---\n/g)) {
    const json = import_yamljs.default.parse(part);
...
function getRegExp(regex) {
  var _a;
  const [, , pattern, modifiers, otherPattern] = (_a = /^(\/(.*?)\/(\w+)?$)|(.*)$/.exec(regex)) != null ? _a : [];
  return new RegExp(pattern != null ? pattern : otherPattern, modifiers != null ? modifiers : "g");
}
/*!
