...
function main() {
  return __async(this, null, function* () {
    var _a;
    const githubApiKey = import_core.default.getInput("github-token") || process.env.GITHUB_TOKEN;
    const repository = import_core.default.getInput("repository") || process.env.GITHUB_REPOSITORY;
    const path = import_core.default.getInput("path") || "service.datadog.yaml";
    ...
    const file = yield getFile(octokit, repository, path);
    const datadog = {
      service: "",
      team: "",
      module: "",
    ...
    if (file) {
      const json = import_yamljs.default.parse(file);
      if (json) {
    ...
    import_core.default.setOutput("service", datadog.service);
    import_core.default.setOutput("team", datadog.team);
    import_core.default.setOutput("module", datadog.module);
    if (datadog.service) {
      import_core.default.exportVariable("DD_SERVICE", datadog.service);
      import_core.default.exportVariable("TFSO_DD_SERVICE", datadog.service);
    }
    if (datadog.tags.length > 0) {
      import_core.default.exportVariable("DD_TAGS", datadog.tags.join(","));
      import_core.default.exportVariable("TFSO_DD_TAGS", datadog.tags.join(","));
    }
  });
}
function getFile(octokit, repository, path) {
  return __async(this, null, function* () {
